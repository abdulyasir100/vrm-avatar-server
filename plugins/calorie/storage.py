...
def init(db_path: str):
    ...
    with _connect() as conn:
        ...
        """)
        # Add `status` column on existing DBs (one-time migration). Values:
        # 'approved' (character said yes), 'overruled' (user ate after they said no).
        existing_cols = [r[1] for r in conn.execute("PRAGMA table_info(meals)").fetchall()]
        if "status" not in existing_cols:
            conn.execute("ALTER TABLE meals ADD COLUMN status TEXT DEFAULT 'approved'")
...
def lookup_food(name: str) -> dict | None:
    """Look up food in the local foods database."""
...
def create_meal(meal_type: str, food_name: str, calories: int = 0,
                ...
                portion: str = "", status: str = "approved") -> dict:
    meal_id = str(uuid.uuid4())[:8]
    now = datetime.now(timezone.utc).isoformat()
    with _lock, _connect() as conn:
        conn.execute(
            """INSERT INTO meals (id, meal_type, food_name, calories, protein_g, carbs_g, fat_g, portion, status, created_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (meal_id, meal_type, food_name, calories, protein_g, carbs_g, fat_g, portion, status, now),
        )
    return {"id": meal_id, "food_name": food_name, "calories": calories, "status": status}
