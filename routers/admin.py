...
class ConfigUpdate(BaseModel):
    ...
    tool_call_mode: Optional[str] = None  # "normal" | "semi_normal" | "semi_off" | "off"
...
async def update_config(req: ConfigUpdate):
    ...
    if req.tool_call_mode is not None and req.tool_call_mode in ("normal", "semi_normal", "semi_off", "off"):
        config.TOOL_CALL_MODE = req.tool_call_mode
        changes.append(f"tool_call_mode: {req.tool_call_mode}")

    # Persist the new values so they survive container rebuilds/deploys.
    if changes:
