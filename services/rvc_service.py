"""RVC Service — Suisei voice conversion via RVC v2.

Converts Kokoro TTS output into Suisei's voice using the
Hoshimachi Suisei RVC v2 model from HuggingFace.

Pipeline: Kokoro TTS WAV -> RVC voice conversion -> Suisei WAV
"""
...
def init_rvc() -> bool:
    ...
    try:
        ...
        _rvc_ready = True
        logger.info(f"[rvc] Suisei RVC ready — model={model_path.name}, device={config.RVC_DEVICE}")
        return True
