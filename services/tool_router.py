...
def setup_default_routes() -> None:
    ...
    # Screen time tool
    register_route(
        r"\b(?:screen\s*time|phone\s*usage|socmed|social\s*media|scrolling|how\s+(?:long|much)\s+(?:on|time))\b",
        ["check_screen_time"],
    )

    # Steps tool
    register_route(
        r"\b(?:steps?|walk(?:ed|ing)?|jalan|langkah|exercise|active|lazy|sedentary)\b",
        ["check_steps"],
    )
