"""TTS Service — supports Qwen3-TTS, HuggingFace Space, and Kokoro-ONNX backends.

Engine priority:
  - qwen3tts: local GPU (PC-only, chichi-speech at port 9090)
  - hfspace:  cloud GPU via HuggingFace ZeroGPU, falls back to kokoro on failure
  - kokoro:   CPU-only ONNX, fast but generic voice
"""
...
def _cleanup_audio_dir():
    ...
    keep = getattr(config, "AUDIO_KEEP_COUNT", 2)
...
def _synthesize_cosyvoice_single(text: str, emotion: str = "neutral", language: str = "en") -> Optional[str]:
    """Synthesize a single chunk with CosyVoice, return audio URL path."""
    import config
    import httpx
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.warning(f"[tts] CosyVoice single error {resp.status_code}: {resp.text[:100]}")
            return None
        ...
        wav_resp = httpx.get(f"{config.COSYVOICE_API_URL}{audio_url}", timeout=10)
        ...
        logger.info(f"[tts] CosyVoice single: {len(wav_resp.content)} bytes ({data.get('duration', '?')}s)")
        return f"/audio/{filename}"

    except Exception as e:
        logger.warning(f"[tts] CosyVoice single failed: {e}")
        return None
