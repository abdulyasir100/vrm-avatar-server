...
from services import llm, tts_service, stt_service, rvc_service, tool_registry, memory, tool_router, costume_registry, background, message_queue, mood, sticker, sensor, plugin_loader
import services.tools
from routers import status, chat, tts, stt, ws, event, costume, memory_router, weather, prayer, admin, stickers, sensor as sensor_router, plugin as plugin_router
import config
...
async def startup():
    ...
    sensor.init(config.MEMORY_DB_PATH)
    ...
    logger.info(f"Task Manager: {tm_status.upper()}")
    logger.info(f"Money Manager: {mm_status.upper()}")
    ...
    background.start()
    logger.info(f"Background: idle talk every {config.IDLE_TALK_INTERVAL_HOURS}h, reminders every {config.TASK_REMINDER_POLL_SECONDS}s")
